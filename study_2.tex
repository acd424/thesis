...
\item{Groups.} This final category relates too if a group was complained about in the ASB log or not. Groups were classified as three or more people. References to families were excluded. For example \say{Four adults having a party in a garden} would be an example of a group.
...
In the explainability section it was found that the the Covid classification method was using elements of the automatically generated text to make the covid complaint prediction. This was again reinforced with the bias metrics where the Covid classifier made more mistakes with the electronic data as it relied on the automatically generated text (e.g. Default) that did not give information on covid complaints, but was rather just correlated with covid complaints.  

The traditional ASB classifier made more errors with the telephone data, and again from Figure \ref{fig:asb_prop} it can be seen that the telephone data predominantly contained logs related to traditional ASB. On this occasion there is no evidence from the explainability section that words are being missused so it cant be certain that this bias is using the same mechanism i.e. there are words or phrases that are distinguishing the telephone data from the electronic data.  A more in depth investigation into error analysis of individual logs and the word clouds for the negative classifications is required to understand the bias mechanism more fully.
...
\section{Summary} This chapter has introduced and tested the use of PTMs with police incident data. The main difference between this study and the previous studies is the type of data analysed and the model used. 
